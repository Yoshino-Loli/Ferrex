...
pub fn find_mono(
    ...
    for folder_name in folder_names.iter() {
        for lib_name in lib_names.iter() {
            ...
            let lib_path = join_dll_path!(
                game_base_path.join(folder_name).join("EmbedRuntime"),
                lib_name
            );
            if lib_path.exists() {
                return Ok(lib_path);
            ...
            let lib_path =
                join_dll_path!(game_data_path.join(folder_name).join("x86_64"), lib_name);
            if lib_path.exists() {
                return Ok(lib_path);
            }

            let lib_path = join_dll_path!(
                game_data_path.join(folder_name).join("EmbedRuntime"),
                lib_name
            );
            if lib_path.exists() {
                return Ok(lib_path);
